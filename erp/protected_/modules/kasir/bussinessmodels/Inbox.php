...
{       
	...
	public function listInbox()
	{
		$criteria=new CDbCriteria;
                ...
                $criteria->select = "cast(p.tanggal as date) as tanggal,pel.nama as pelanggan, cast(p.tanggal as date)|| '' ||pelangganid as id ";
                $criteria->alias = "p";
                ...
                $criteria->condition = "statuspenjualan=false AND issendtokasir=true AND isdeleted=false AND jenispembayaran='tunai' and p.lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];
        ...
        public function listDetailInbox($pelangganid,$tanggal)
        {
            $sql ="select d.nama as divisi,b.nama as barang,pb.jumlah,pb.hargatotal,pb.hargasatuan,pb.kategori,pb.id as id
                    from transaksi.penjualanbarang pb inner join master.divisi d on d.id=pb.divisiid
                        inner join master.barang b on b.id=pb.barangid
                            where pb.jenispembayaran='tunai' and pb.statuspenjualan=false and pb.pelangganid=$pelangganid and cast(pb.tanggal as date)='$tanggal' and pb.lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];
        ...
        public function detailFormFaktur($pelangganid,$tanggal)
        {
            ...
            $sql ="select d.nama as divisi,b.nama as barang,pb.box,pb.jumlah,pb.hargatotal,pb.hargasatuan,pb.kategori,pb.id as id,b.id as barangid
                    from transaksi.penjualanbarang pb inner join master.divisi d on d.id=pb.divisiid
                        inner join master.barang b on b.id=pb.barangid
                            where pb.statuspenjualan=false and pb.jenispembayaran='tunai' and pb.pelangganid=$pelangganid and cast(pb.tanggal as date)='$tanggal' and pb.lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];            
        ...
        public function detailFormFakturCetak($pelangganid,$tanggal,$pembelianke)
        {
            ...
            /*$sql ="select d.nama as divisi,b.nama as barang,pb.box,pb.jumlah,pb.hargatotal,pb.hargasatuan,pb.kategori,pb.id as id,b.id as barangid
                    from transaksi.penjualanbarang pb inner join master.divisi d on d.id=pb.divisiid
                        inner join master.barang b on b.id=pb.barangid
                            where pb.statuspenjualan=true and pb.jenispembayaran='tunai' and pb.pelangganid=$pelangganid and cast(pb.tanggal as date)='$tanggal' and pb.lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];   */
        ...
        {
            $connection =Yii::app()->db;
            $sql = "update transaksi.penjualanbarang set statuspenjualan=TRUE "
                    . "     where statuspenjualan=FALSE and jenispembayaran='tunai' and  pelangganid=$pelangganid "
                    . "         and cast(tanggal as date)='$tanggal' and lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];
            $command = $connection->createCommand($sql);
            $command->execute();
        }
        
        public function setJenisPembayaran($pelangganid,$tanggal)
        {
            $connection =Yii::app()->db;
            $sql = "update transaksi.penjualanbarang set jenispembayaran='kredit' where  pelangganid=$pelangganid and cast(tanggal as date)='$tanggal' and lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];
            $command = $connection->createCommand($sql);
            $command->execute();
        }
