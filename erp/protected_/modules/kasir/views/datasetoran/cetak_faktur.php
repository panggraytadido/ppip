...
class FPDF_AutoWrapTable extends FPDF{
 ...
 public function FancyTable($header, $data){                      
	...
	$this->Cell(120, 0, 'Jl. Tulang Kuning No. 04 RT.02 RW.05', 0, 0,'L');
                 ...
                 //$this->Cell(90, -25, 'Parung, '.$tanggalnya.'', 0, 0,'C');
  ...
  $this->SetFont('Courier','','10');
  		//$sqldis = mysql_query("SELECT * FROM tb_diskon WHERE faktur='".$_POST['faktur']."' ") or die(mysql_error());
		//while ($res=mysql_fetch_array($sqldis)) {
  ...
  $dataku = Datasetoran::model()->detailFormFaktur($_GET['pelangganid'],$_GET['tanggal']);
  $hargatotal=array();
...
$tanggal = $_GET['tanggal'];
    ...
    $faktur = Faktur::model()->find("pelangganid=".intval($_GET['pelangganid'])." and cast(tanggal as date)='$tanggal' and pembelianke=".$_GET['pembelianke']);
    ...
    $this->Cell(30, 6, 'Sisa', 1, 0,'L');
    $this->Cell(0, 6, ''.number_format($faktur->sisa).'', 1, 1,'R');
