...
class User extends CActiveRecord
{
    ...
    public function relations()
    {
        ...
        $relations['userskaryawan'] = array(self::HAS_ONE, 'Userskaryawan', 'usersid');
    ...
    public function defaultScope()
    {
        return CMap::mergeArray(Yii::app()->getModule('user')->defaultScope,array(
            'alias'=>'user',
            'select' => 'user.id, user.username, user.email, user.create_at, user.lastvisit_at, user.superuser, user.status, user.filename',
        ));
    }
    ...
    public function search()
    {
        ...
        $criteria->compare('CAST(create_at AS DATE)',$this->create_at);
        $criteria->compare('lastvisit_at',$this->lastvisit_at);
        $criteria->compare('superuser',$this->superuser);
        $criteria->compare('status',$this->status);
        
        $criteria->alias = 'u';
        $criteria->join = 'LEFT JOIN public.userskaryawan AS k ON u.id=k.usersid '; 
        $criteria->join .= ' LEFT JOIN master.divisi AS d ON k.divisiid=d.id '; 
        $criteria->join .= ' LEFT JOIN transaksi.lokasipenyimpananbarang AS lpb ON lpb.id=k.lokasipenyimpananbarangid '; 
        
        if(isset($_GET['User']['divisi']))
        {
            $criteria->compare('d.nama',$_GET['User']['divisi'],true);
        }
        if(isset($_GET['User']['lokasi']))
        {
             $criteria->addCondition("lpb.id=".$_GET['User']['lokasi']);
        }                    
        
        $sort = new CSort();
        $sort->attributes = array(
            'defaultOrder'=>'superuser',
            'username',
            'divisi'=>array(
                              'asc'=>'d.nama',
                              'desc'=>'d.nama desc',
