...
class Penjualanbaranggudang extends Penjualanbarang
{          
    ...
    public function listBarang()
    {
        ...
        if(isset($_GET['Penjualanbaranggudang']))
        {
                    ...
                    if($_GET['Penjualanbaranggudang']['tanggal']!='')
                        $criteria->compare('pb.tanggal', date("Y-m-d", strtotime($_GET['Penjualanbaranggudang']['tanggal'])));
        }
    ...
    public function getTotalPenjualanBarangPerTanggal()
    {
       $divisiid = Divisi::model()->find("kode='1'")->id;
       $tanggal = date("Y-m-d", time());
        $connection=Yii::app()->db;
      $sql="select sum(jumlah) as total from transaksi.penjualanbarang "
                . "where divisiid=$divisiid and statuspenjualan=true and cast(tanggal as date)='$tanggal'";					
