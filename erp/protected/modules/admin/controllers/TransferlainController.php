...
class TransferlainController extends Controller
{
                    ...
                    if($valid)
                    {
                        ...
                        try{
                                ...
                                $sql ="select max(id) as id 
											from transaksi.transfer where rekeningid=".$_POST['Transferlain']['rekeningid'];           
                                $data=$connection->createCommand($sql)->queryRow();                                                
                                $id = $data["id"];
								if($id!="")
								{
									$criteria = new CDbCriteria;                        
									$criteria->condition="rekeningid=".$_POST['Transferlain']['rekeningid']."  AND id=".$id;
								...
								else
								{
									...
									if($saldo!=0)
									{
										...
										$transfer->saldo = $saldo-$_POST['Transferlain']['jumlah'];     
	...
	public function actionGetNorek()
	{
		if (Yii::app ()->request->isAjaxRequest) {                
			...
			$saldo = $this->GetSaldoRek(intval( $_POST['rekeningid']));
			echo CJSON::encode(array
						(
							'result'=>'OK',
							'data'=>$model,
	...
	{
		$transferLain = Transferlain::model()->findByPk($id);
		$transfer = Transfer::model()->findByPk($transferLain->transferid)->delete();
