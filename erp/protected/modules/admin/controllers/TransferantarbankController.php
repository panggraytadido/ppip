...
class TransferantarbankController extends Controller
{
        ...
        public function actionCreate()
	{
		...
		if(isset($_POST['Transferantarbank']))
		{
                    ...
                    if($valid)
                    {
                        ...
                        $sql ="select max(id) as id from transaksi.transfer where rekeningid=".$_POST['Transferantarbank']['rekeningid'];           
                        ...
                        if($id!='')
                        {
                            ...
                            $criteria->condition="rekeningid=".$_POST['Transferantarbank']['rekeningid']." AND id=".$id;
                        ...
                        //rekening tujuan 
                        $connection=Yii::app()->db;
                        $sql ="select max(id) as id from transaksi.transfer where rekeningid=".$_POST['Transferantarbank']['rekeningtujuanid'];           
                        $data=$connection->createCommand($sql)->queryRow();                                  
                        $id = $data["id"];                      
                        ...
                        {
                            $criteria = new CDbCriteria;                        
                            $criteria->condition="rekeningid=".$_POST['Transferantarbank']['rekeningtujuanid']." AND id=".$id;
                        ...
                        {                            
                            ...
                            if($rekeningTujuan->save())
                            {
                                ...
                                $model->transferidrekeningasal=$rekeningAsal->id;
								$model->transferidrekeningtujuan=$rekeningTujuan->id;
                                if($model->save())
                                {
        ...
        public function actiongetNoRekening()
        {
            if (Yii::app ()->request->isAjaxRequest) {
                ...
                {
                    $criteria = new CDbCriteria;                        
                    $criteria->condition="rekeningid=".intval($_POST['rekeningid'])." AND id=".$id;
	...
	{
		$transferAntaBank = Transferantarbank::model()->findByPk($id);
		$data =array($transferAntaBank->transferidrekeningasal,$transferAntaBank->transferidrekeningtujuan);
		foreach($data as $key=>$value)
		{
			Transfer::model()->findByPk($value)->delete();
		}				
		$model=Transferantarbank::model()->findByPk($id)->delete;
