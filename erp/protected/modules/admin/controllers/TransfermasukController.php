	...
	{           
        ...
        public function actiongetNoRekening()
        {
            if (Yii::app ()->request->isAjaxRequest) {
                ...
                $sql ="select max(id) as id from transaksi.transfer where rekeningid=".intval($_POST['id']);           
        ...
        public function actionPopupfaktur()
        {
            if (Yii::app ()->request->isAjaxRequest) {
                ...
                $criteria->order='cast(tanggal as date) asc';
        ...
        public function actionSimpanSetoran()
        {
            if (Yii::app ()->request->isAjaxRequest) {                                            
                 $transaction = Yii::app()->db->beginTransaction();
                    try{
						//print("<pre>".print_r($_POST['tanggaltransfer'],true)."</pre>");
						//die;
                            for($i=0;$i<count($_POST['fakturid']);$i++)
                            {
                                ...
                                if($_POST['bayar'][$i]!='')
                                {
                                    ...
                                    $setoran->tanggalsetoran=Yii::app()->DateConvert->ConvertTanggal($_POST['tanggaltransfer'][$i]);
                                    ...
                                    if($setoran->save())
                                    {
										$faktur->sisa = $faktur->sisa - $_POST['bayar'][$i];
										$faktur->bayar = $faktur->bayar + $_POST['bayar'][$i];
