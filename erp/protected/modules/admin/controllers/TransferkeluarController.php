...
class TransferkeluarController extends Controller
{
        ...
        {           
                        $model=new Transferkeluar('search');
                        $model->unsetAttributes();

                        if(isset($_GET['Transferkeluar']))
                                        $model->attributes = $_GET['Transferkeluar'];
                ...
                {		
					...
					if($_POST['Transferkeluar']["jenisTransfer"]==3)
					{
						...
						try{
									...
									$model2->jenistransferid=3; // transfer ke supplier
									...
									if($model2->save())
									{
											//select saldo rekening
											$connection=Yii::app()->db;
											$sql ="select max(id) as id from transaksi.transfer 
														where rekeningid=".intval($_POST['Transferkeluar']['rekening'])." AND supplierid=0";           
											$data=$connection->createCommand($sql)->queryRow();
											$id = $data["id"];                        
											if($id!="")
											{
