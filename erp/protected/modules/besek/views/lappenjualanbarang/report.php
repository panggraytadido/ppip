...
class FPDF_AutoWrapTable extends FPDF{
 ...
 function FancyTable($header, $data){
	...
	$this->Ln(5);
	$this->SetFont("", "B", 9);
	$this->Cell(0, 5, 'LAPORAN RINCIAN PENGELUARAN BESEK', 0, 0,'C');
	$this->Ln(10);
        ...
        $w = array(5,20,25, 45, 30, 20, 20, 20);
        ...
        $this->Cell(125, 6, 'Total', 1, 0,'C');      
        // Load Data dari Database       
        if($_GET['status']==1)
        {
            $t = Lappenjualanbarangbesek::model()->getTotalPenjualanBarangPerTanggal($_GET['pilihan']);
            
        }
        else
        {
            $t = Lappenjualanbarangbesek::model()->getTotalPenjualanBarangPerBulan($_GET['pilihan']);           
        }
  ...
  //Fungsi Untuk Membuat Footer
	function Footer()
	{
        ...
        if($_GET['status']==1)
        {
            $data = Lappenjualanbarangbesek::model()->getPenjualanBarangPerTanggal($_GET['pilihan']);
            //print_r($data);
        }
        else
        {
            $data = Lappenjualanbarangbesek::model()->getPenjualanBarangPerBulan($_GET['pilihan']);
        }
