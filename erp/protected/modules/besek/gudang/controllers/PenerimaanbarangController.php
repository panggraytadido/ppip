...
class PenerimaanbarangController extends Controller
{
	...
	public function filters()
	{
        ...
        public function actionCreate()
        {
            ...
            $this->performAjaxValidation($modelPenerimaan, $modelBongkarmuat);
            
            if(isset($_POST['Gudangpenerimaanbarang'],$_POST['Bongkarmuat']))
            {
							...
							if($model->save())
							{
										...
										if($_POST['Bongkarmuat']['jumlahkaryawan']!='0') // jika input jumlah karyawan, insert data bongkar muat 
										{
											...
											for($i=0;$i<count($karyawanid);$i++)
											{
												...
												$modelBongkar->tanggal = $_POST['Gudangpenerimaanbarang']['tanggal'];
												$modelBongkar->createddate = $createddate;
												$modelBongkar->userid = Yii::app()->user->id;
												$modelBongkar->penerimaanbarangid = $model->getPrimaryKey();
												$modelBongkar->divisiid = $_POST['Gudangpenerimaanbarang']['divisiid'];
												$modelBongkar->save();
											}
										}
						...
						if($modelPenerimaan->save())
						{
							if($_POST['Bongkarmuat']['jumlahkaryawan']!='0') // jika input jumlah karyawan, insert data bongkar muat 
							{
								...
								foreach($modelBongkarmuat as $r) // delete/update karyawan lama
								{
									...
									{
										$deleteBongkarmuat = Bongkarmuat::model()->find('penerimaanbarangid='.$modelPenerimaan->id.' and karyawanid='.$r->karyawanid);
									...
									else // jika masih ada di input baru, update
									{
										...
										$updateBongkarmuat = Bongkarmuat::model()->find('penerimaanbarangid='.$modelPenerimaan->id.' and karyawanid='.$r->karyawanid);
										$updateBongkarmuat->upah = $_POST['Bongkarmuat']['upah'];
										$updateBongkarmuat->tanggal = $_POST['Gudangpenerimaanbarang']['tanggal'];
										$updateBongkarmuat->divisiid = Yii::app()->session['divisiid'];
								...
								*/
								...
								$bngkarMuat = Bongkarmuat::model()->findAll("penerimaanbarangid=".$modelPenerimaan->id);								
								for($x=0;$x<count($karyawanBaru);$x++)
								{
										if($karyawanBaru[$x]!=isset($bngkarMuat[$x]))
										{
											...
											$model->penerimaanbarangid = $modelPenerimaan->id;
