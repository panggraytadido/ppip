                        ...
                        if($model->save())
                        {
                            ...
                            if($_POST['Bongkarmuat']['jumlahkaryawan']!='0') // jika input jumlah karyawan, insert data bongkar muat 
                            {
                                ...
                                for($i=0;$i<count($karyawanid);$i++)
                                {
                                    ...
                                    $modelBongkar->tanggal = $_POST['Gudangpemindahanbarang']['tanggal'];
                                    $modelBongkar->createddate = $createddate;
                                    $modelBongkar->userid = Yii::app()->user->id;
                                    $modelBongkar->pemindahanbarangid = $model->getPrimaryKey();
                    ...
                    if($modelPemindahan->save())
                    {
                        ...
                        if($_POST['Bongkarmuat']['jumlahkaryawan']!='0') // jika input jumlah karyawan, insert/update data bongkar muat 
                        {
                            ...
                            foreach($modelBongkarmuat as $r) // delete/update karyawan lama
                            {
                                ...
                                {
                                    $deleteBongkarmuat = Bongkarmuat::model()->find('pemindahanbarangid='.$modelPemindahan->id.' and karyawanid='.$r->karyawanid);
                                ...
                                else // jika masih ada di input baru, update
                                {
                                    ...
                                    $updateBongkarmuat = Bongkarmuat::model()->find('pemindahanbarangid='.$modelPemindahan->id.' and karyawanid='.$r->karyawanid);
                                    $updateBongkarmuat->upah = $_POST['Bongkarmuat']['upah'];
                                    $updateBongkarmuat->tanggal = $_POST['Gudangpemindahanbarang']['tanggal'];
                            ...
                            for($i=0;$i<count($karyawanBaru);$i++) // insert karyawan baru
                            {
                                if (!in_array($karyawanBaru[$i], $karyawanLama))
                                {
                                    $dataKaryawan = array(
                                        ...
                                        'pemindahanbarangid' => $modelPemindahan->id,
                                        'upah' => $_POST['Bongkarmuat']['upah'],
                                        'tanggal' => $_POST['Gudangpemindahanbarang']['tanggal'],
                                    ...
                                    $cekExistKaryawan = $modelPemindahan->cekExistKaryawan($dataKaryawan); // cek exist karyawan (menghindari double input)

                                    if($cekExistKaryawan==0)
                                    {
                                        $modelBongkar = new Bongkarmuat;
                                        $modelBongkar->karyawanid = $karyawanBaru[$i];
                                        $modelBongkar->pemindahanbarangid = $modelPemindahan->id;
                                        $modelBongkar->upah = $_POST['Bongkarmuat']['upah'];
                                        $modelBongkar->tanggal = $_POST['Gudangpemindahanbarang']['tanggal'];
                                        $modelBongkar->createddate = $createddate;
                                        $modelBongkar->userid = Yii::app()->user->id;
