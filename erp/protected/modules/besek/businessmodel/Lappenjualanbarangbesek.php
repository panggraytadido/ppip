...
class Lappenjualanbarangbesek extends Penjualanbarang
{
        public $pilih;
	public $tanggal;
	public $bulan;
        ...
        public static function model($className=__CLASS__)
	{
        ...
        public function getPenjualanBarangPerTanggal($tanggal)
        {
            ...
            $sql="select s.namaperusahaan as supplier,b.nama as barang,
                    p.nama as pelanggan,cast(pb.tanggal as date) as tanggal ,pb.jumlah,pb.hargasatuan,pb.hargatotal
                                ...
                                inner join master.pelanggan p on p.id=pb.pelangganid
                                        inner join master.supplier s 
                                                ...
                                                where EXTRACT(MONTH FROM tanggal)='$bulan' and EXTRACT(YEAR FROM tanggal)='$currYear'
                                                        and pb.divisiid=$divisiid and pb.lokasipenyimpananbarangid=$lokasi order by cast(pb.tanggal as date) asc";					
            $data=$connection->createCommand($sql)->queryAll();
            return $data;
        }
