...
<input type="hidden" name="waktuupdate" value="<?php echo date("Y-m-d H:", time()); ?>" />
        ...
        <?php echo $form->datePickerGroup($modelPenjualan, 'tanggal', array(
                                            ...
                                            'widgetOptions' => array(
                                                        'htmlOptions'=>array(
                                                                'id'=>'Hygienispenjualanbarang_tanggal',
                                                                'readonly'=>'readonly',
        ...
        <?php echo $form->textFieldGroup($modelPenjualan, 'jumlah',array(
                                    ...
                                    'widgetOptions' => array(
                                            'htmlOptions' => array(
                                                    ...
                                                    'id'=>'Hygienispenjualanbarang_jumlah'
                                            )
                                    ),
        ...
        <div class="form-group">
            <label class="col-sm-5 control-label required" for="Hygienispenjualanbarang_kategori">Kategori</label>
            <div class="col-sm-4">
                <input type="radio" name="Hygienispenjualanbarang[kategori]" id="kategori1" onchange="hitungHargaBarang();" value="1" <?php echo ($modelPenjualan->kategori==1) ? 'checked="checked"' : ''; ?> /> R.Ikan 
                <input type="radio" name="Hygienispenjualanbarang[kategori]" id="kategori2" onchange="hitungHargaBarang();" value="2" <?php echo ($modelPenjualan->kategori==2) ? 'checked="checked"' : ''; ?> /> Grosir 
            </div>
        </div>
       ...
       <?php echo $form->textFieldGroup($modelPenjualan, 'hargasatuan', array(
                                            ...
                                            'widgetOptions' => array(
                                                        'htmlOptions'=>array(
                                                                'readonly'=>'readonly',
                                                                'id'=>'Hygienispenjualanbarang_hargasatuan_input',
                                                            )
                                                )
                                        )
            ); 
       ...
       <?php echo $form->textFieldGroup($modelPenjualan, 'hargatotal', array(
                                            ...
                                            'widgetOptions' => array(
                                                        'htmlOptions'=>array(
                                                                'readonly'=>'readonly',
                                                                'id'=>'Hygienispenjualanbarang_hargatotal_input',
                                                            )
                                                )
                                            )
                                        ); 
            ...
            array(
                ...
                'success'=>'js:function(data) 
                 {    
                   ...
                   if(data.result==="OK")
                   {             
                      location.href="'.Yii::app()->createUrl("hygienis/penjualanbarang/view", array("id"=>$modelPenjualan->id)).'";
                   }
                   else
                   {                        
...
<script type="text/javascript">
    ...
    function getLokasiBarang(barangid)
    {
        if(barangid!='')
        {
            $.ajax({
                        dataType: 'json',
                        type: 'POST',
                        url: '<?php echo Yii::app()->baseurl; ?>/hygienis/penjualanbarang/getlokasibarang',
                        data: { barangid : barangid},
                        success: function (data) {
                            
                            var x = document.getElementById('Hygienispenjualanbarang_lokasipenyimpananbarangid');

                            // hapus option
                            $('#Hygienispenjualanbarang_lokasipenyimpananbarangid option[value!=""]').remove();
                            //$('#Hygienispenjualanbarang_lokasipenyimpananbarangid').children('option:not(:first)').remove();;
    ...
    {
        var e = document.getElementById("Hygienispenjualanbarang_barangid");
        ...
        if(barangid!='')
        {
            $.ajax({
                        dataType: 'json',
                        type: 'POST',
                        url: '<?php echo Yii::app()->baseurl; ?>/hygienis/penjualanbarang/getsupplierbarang',
                        data: { barangid : barangid},
                        success: function (data) {
                            
                            var x = document.getElementById("Hygienispenjualanbarang_supplierid");

                            // hapus option
                            $('#Hygienispenjualanbarang_supplierid option[value!=""]').remove();
    ...
    function cekStock()
    {
        var jumlah = document.getElementById("Hygienispenjualanbarang_jumlah").value;
        
        var e = document.getElementById("Hygienispenjualanbarang_barangid");
        var barangid= e.options[e.selectedIndex].value;

        var f = document.getElementById("Hygienispenjualanbarang_lokasipenyimpananbarangid");
        var lokasiid= f.options[f.selectedIndex].value;
        
        var g = document.getElementById("Hygienispenjualanbarang_supplierid");
        ...
        if(barangid!='' && lokasiid!='' && supplierid!='' && jumlah.trim()!='' && isNormalInteger(jumlah))
        {
            $.ajax({
                        dataType: 'json',
                        type: 'POST',
                        url: '<?php echo Yii::app()->baseurl; ?>/hygienis/penjualanbarang/cekstock',
                        ...
                        success: function (data) {
                            ...
                            {
                                document.getElementById("Hygienispenjualanbarang_jumlah").value = '';
    ...
    {
        var tgl = document.getElementById("Hygienispenjualanbarang_tanggal").value;
        var jumlahAbsen = 0;
        ...
        if(tgl!='')
        {
            $.ajax({
                    type: 'POST',
                    url: '<?php echo Yii::app()->baseurl; ?>/hygienis/penjualanbarang/getjumlahabsen',
                    data: { tanggal : tgl},
    ...
    function tambahKaryawan()
    {
        ...
        if(tgl!='')
        {
            $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: '<?php echo Yii::app()->baseurl; ?>/hygienis/penjualanbarang/getkaryawanabsen',
                    data: { tanggal : tgl},
                    success: function (data) {
    ...
    {
        if(document.getElementById("Hygienispenjualanbarang_barangid").value!='')
        {
            hargaBarang = 0;
            ...
            $.ajax({
                    type: 'POST',
                    url: '<?php echo Yii::app()->baseurl; ?>/hygienis/penjualanbarang/hargabarang',
                    data: { barangid : document.getElementById("Hygienispenjualanbarang_barangid").value, kategori : kategori, supplierid : document.getElementById("Hygienispenjualanbarang_supplierid").value},
                    success: function (data) {
                        document.getElementById("Hygienispenjualanbarang_hargasatuan_input").value = data;
                        hitungHargaTotal();
                    },
                    error: function () {
        ...
        {
            document.getElementById("Hygienispenjualanbarang_hargasatuan_input").value = '0';
            hitungHargaTotal();
        }
    }
    ...
    {
        rp = 0;
        rp = document.getElementById("Hygienispenjualanbarang_jumlah").value * document.getElementById("Hygienispenjualanbarang_hargasatuan_input").value;
        document.getElementById("Hygienispenjualanbarang_hargatotal_input").value = rp;
    }
    
    function hitungRupiah()
    {
        rp = 0;
        rp = (document.getElementById("Hygienispenjualanbarang_jumlah").value * 30) / document.getElementById("Bongkarmuat_jumlahkaryawan").value;
