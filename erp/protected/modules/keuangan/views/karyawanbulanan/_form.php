...
<div style="float:left">            
    <?php 
        echo CHtml::ajaxSubmitButton('Simpan',CHtml::normalizeUrl(array('karyawanbulanan/create','render'=>true)),
            array(
                'dataType'=>'json',
                'type'=>'POST',                      
                'success'=>'js:function(data) 
                 {    
                   if(data.result==="OK")
                   {             
                      location.href="'.Yii::app()->baseurl.'/keuangan/karyawanbulanan";
                   }
                   else
                   {                        
...
function checkexist(p)
{
	...
	if(karyawanid!="")
	{
		$.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: '<?php echo Yii::app()->baseurl; ?>/keuangan/karyawanbulanan/checkexist',
                    data: { karyawanid : karyawanid},
                    success: function (data) {
                         if(data.result==1)
						 {
							alert('data gaji karyawan bulanan sudah ada');								
						 }								 
                    },
                    error: function () {
