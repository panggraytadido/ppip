...
$this->pageTitle = "Laporan Penerimaan Barang - Gudang";
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="row">
        <div class="col-lg-12">                     
            <div class="ibox float-e-margins">
                 <div class="ibox-title">
                    <div class="pull-left">
                        <b>Gudang - Laporan Penerimaan Barang</b>
                    </div>                   
                </div>
        ...
        ?>
        <div style="display:none" id="Lappenerimaanbaranggudang_pilih_em_" class="help-block error"></div>
    ...
    <?php
                ...
                echo $form->dropDownListGroup($model, 'tahun',array(
                                    'wrapperHtmlOptions'=>array('class'=>'col-sm-4'),
                                    'widgetOptions' => array(
                                            'data' =>array($tahun=>$tahun,$tahunSebelumnya=>$tahunSebelumnya),
                                            'htmlOptions' => array(
                                                    'prompt'=>'Pilih Tahun',                                                    
                                            )
                                    )
             ...
             array(
                 ...
                 'success'=>'js:function(data) 
                  {                                       
                    if(data.result==="OK")
                    {             
                        //location.href="'.Yii::app()->baseurl.'/gudang/lappenerimaanbarang/print/pilihan/" + data.pilihan+"/status/" +data.status    	             			             			             			             			             			             
						window.open("'.Yii::app()->baseurl.'/gudang/lappenerimaanbarang/print/pilihan/" + data.pilihan+"/status/" +data.status+"/tahun/"+data.tahun, "_blank");  
                    }                  
                    else
                    {                        
...
function active()
{
    var option = $('#Lappenerimaanbaranggudang_pilih option:selected').val();    
    if(option==1)
    {
        $('#tanggal').show();    
        $('#bulan').hide();  
...
function clearErr()
{
    $('#Lappenerimaanbaranggudang_pilih_em_').empty();
}
</script>