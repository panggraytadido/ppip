...
class PenjualanbarangController extends Controller
{
        ...
        public function actionCreate()
        {
            ...
            if(isset($_POST['Gudangpenjualanbarang'],$_POST['Bongkarmuat']))
            {
                ...
                if( ($valid && is_numeric($_POST['Bongkarmuat']['upah'])) && (isset($_POST['karyawanid']) && $_POST['Bongkarmuat']['jumlahkaryawan']==count($_POST['karyawanid'])))
                {
					...
					// cek faktur untuk menentukan pembelian ke, klw difaktur sudah ada maka pembelianke+1
					$pelangganid = $_POST['Gudangpenjualanbarang']['pelangganid'];
					$tanggal = $_POST['Gudangpenjualanbarang']['tanggal'];
										
					$cekFaktur = Faktur::model()->findAll("pelangganid=".$pelangganid." 
                    ...
                    if($cekExist==0)
                    {
						...
						$criteria->select = "max(pembelianke) as pembelianke";
						$criteria->condition="pelangganid=".$pelangganid." AND cast(tanggal as date)='$tanggal' AND statuspenjualan=false AND issendtokasir=true AND lokasipenyimpananbarangid=".Yii::app()->session['lokasiid'];
						$penjualan = Inbox::model()->find($criteria);
	...
	//                            }                      
        ...
        public function actionCekstock()
        {
            ...
            $criteria->condition = 'statuspenjualan = false AND issendtokasir=true AND isdeleted=false';
