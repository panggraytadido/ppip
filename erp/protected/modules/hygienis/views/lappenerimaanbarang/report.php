...
class FPDF_AutoWrapTable extends FPDF{
 ...
 function FancyTable($header, $data){
	...
	$this->Ln(5);
	$this->SetFont("", "B", 9);
	$this->Cell(0, 5, 'LAPORAN RINCIAN PENERIMAAN HYGIENIS', 0, 0,'C');
	$this->Ln(10);
        ...
        $this->Cell(115, 6, 'Total', 1, 0,'C');      
        // Load Data dari Database       
        if($_GET['status']==1)
        {
            $t = Lappenerimaanbaranghygienis::model()->getTotalPenerimaanBarangPerTanggal($_GET['pilihan']);            
        }
        else
        {
            $t = Lappenerimaanbaranghygienis::model()->getTotalPenerimaanBarangPerBulan($_GET['pilihan']);           
        }
  ...
  //Fungsi Untuk Membuat Footer
	function Footer()
	{
        ...
        if($_GET['status']==1)
        {
            $data = Lappenerimaanbaranghygienis::model()->getPenerimaanBarangPerTanggal($_GET['pilihan']);
            //print_r($data);
        }
        else
        {
            $data = Lappenerimaanbaranghygienis::model()->getPenerimaanBarangPerBulan($_GET['pilihan']);
        }
        ...
        if(count($data)>0)
        {
            ...
            for($i=0;$i<count($data);$i++)
            {
                ...
                @$d[] .=  $no++."|".$data[$i]['tanggal']."|".$data[$i]['supplier']."|".$data[$i]['barang']."|".$data[$i]['jumlah']."|".number_format( $data[$i]['hargamodal'] , 0 , '' , '.' )."|" . number_format( $data[$i]['jumlah']*$data[$i]['hargamodal'] , 0 , '' , '.' );  
            }
